...
GameState.prototype.preload = function() {
    this.game.load.image('player', '/assets/imgs/owlbear.png');
};
    ...
    for(var i = 0; i < NUMBER_OF_FOLLOWERS; i++) {
        var f = this.game.add.existing(
            new Follower(this.game,
                ...
                this.game.height/2,
                this.game.input /* the previous follower or pointer */
            )
        );
    }
...
var Follower = function(game, x, y, target) {
    ...
    this.MAX_SPEED =        250 * Math.random(); // pixels/second
    this.MIN_DISTANCE =     32  * Math.random(); // pixels
};
